	...
	for i := 0; i < 10; i++ {
		for _, keywordItem := range keywordsList {
			if len(keywordItem.Keyword) > 3 && rand.Int63n(100) < 30 {
				...
				if len(results.SubtitleDTO) > 0 {
					_, err = s.GetContent(&segments.SearchSubtitleInput{
						SubtitleId: results.SubtitleDTO[rand.Int63n(int64(len(results.SubtitleDTO)))].ID,
